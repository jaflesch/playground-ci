...
export const getOffensiveMatchups = (index: TypesIndex) => {
  const normal = [];
  const noEffect = [];
  const superEffective = [];
  const notEffective = [];

  for (let i = 0; i < TYPES_MATCHUP_MATRIX[index].length; i++) {
    switch (TYPES_MATCHUP_MATRIX[index][i]) {
      case 0:
        noEffect.push(i);
        break;
      case 1:
...
export const getDefensiveMatchups = (index: TypesIndex) => {
  const normal = [];
  const noEffect = [];
  const superEffective = [];
  const notEffective = [];
  ...
  for (let i = 0; i < TYPES_MATCHUP_MATRIX.length; i++) {
    switch (TYPES_MATCHUP_MATRIX[i][index]) {
      case 0:
        noEffect.push(i);
        break;
      case 1:
