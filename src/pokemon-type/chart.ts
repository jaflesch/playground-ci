...
export const getTypeOffensive = (index: PokemonTypesArrayIndex) => {
  const normal = [];
  const noEffect = [];
...
export const getTypeDefensive = (index: PokemonTypesArrayIndex) => {
  const normal = [];
  const noEffect = [];
  const superEffective = [];
  const notEffective = [];
...
export const getPokemonTypeMatchups = (pokemon: Pokemon, verbose?: boolean) => {
  ...
  for (const t of types) {
    ...
    const proscons = getTypeOffensive(t);
  ...
  //     types,
  //   },
  //   types.map((ttt) => format.index2name(ttt)),
  // );
  // console.log('@@ BREAKDOWN >> offensive', JSON.stringify(offensive));
  // console.log('@@ BREAKDOWN >> defensive', JSON.stringify(breakdown));
...
export const PokemonTypeChart = {
  getOffenses: getTypeOffensive,
  getDefenses: getTypeDefensive,
  getAtkPros: () => {},
  getAtkCons: () => {},
  getDefPros: () => {},
  getDefCons: () => {},
